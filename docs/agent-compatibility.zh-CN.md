...
- `mcp-readonly`：只要批准写入与校验闭环不完整，就返回 zero-write dry-run 和待批准的改动计划，绝不声称修改了 vault。
