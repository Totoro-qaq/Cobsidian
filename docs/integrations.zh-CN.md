...
| Level | 接入路径 |
|---|---|
| `full-local` | 使用 MCP 检查，并具备批准后的本地写入与校验路径。 |
| `filesystem-only` | 不使用 MCP，直接通过本地脚本、文件编辑和校验执行。 |
| `mcp-readonly` | 用 Cobsidian MCP 扫描和 dry-run；因为没有写入路径，只返回改动计划。 |
| `chat-only` | 返回可移植草稿，或请求可用 vault/config 路径；不声称完成扫描或写入。 |

这是 zero-write MCP 设计：Cobsidian MCP server 只提供检查与规划，dry-run 始终返回 `writes: []`，也不会因为传入 `capability_level` 而获得写权限。`ready` 表示当前 host 完成检查并能在批准后继续，不表示已经写入。
